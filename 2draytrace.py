...
param = np.genfromtxt('params.txt', skip_header = 1, dtype = sp.intp)
params = np.ndarray.astype(param, dtype = sp.intp)
print (params)
...
#        print thetax[ray,nscreen]
	...
	for ray in range(nrays):
		...
		#omega[ray] = 2*np.pi*omega[ray]*vel/(0.37)
		omega[ray] += (thetax[ray,i])
		xdelay = ((thetax[ray,i]**2)*dz)/(2*(3e8)) #seconds
		ydelay = ((thetay[ray,i]**2)*dz)/(2*(3e8)) #seconds
		tau[ray] += np.sqrt(xdelay**2+ydelay**2)
...
def main():
	...
	vel = velocity # Meters per second
	nscreen = numscreens
	trials = trialnum # Number of raypaths
	...
	for i in range(trials):
		plt.plot(pathx[i,1]/(10**11),pathy[i,1]/(10**11), c='black', marker='o')
	...
	#td = ax1.hist2d(x, y, bins=50, cmap=plt.cm.BuGn_r, weights=sec_amp)
